...
- **Why it matters:** it is fast, surprisingly strong, and a foundation for many modern methods (regularization, kernels, neural networks).
...
> In machine learning, we treat the “assumptions” behind a model as *inductive bias*—a preference for simpler functions that often generalize well.
...
- **Training loss (MSE):**
$$
\operatorname{MSE}_{\text{train}} = \frac{1}{n}\sum_{i=1}^n (y_i-\hat y_i)^2 = \frac{\text{RSS}}{n}.
$$

- **Why MSE?** It is smooth and convex for linear models, which makes optimization and analysis convenient.

### Closed form vs optimization

For linear regression, the ERM problem has a closed-form solution (shown below).  
However, in modern machine learning we often fit models by **gradient-based optimization**, because it scales well to large datasets and extends directly to regularized models and neural networks.

If we write $X$ for the (centered) design matrix and minimize
$$
...
$$
and gradient descent updates are
$$
\beta^{(t+1)}=\beta^{(t)}-\eta\,\nabla_\beta L(\beta^{(t)}).
$$
...
In machine learning, a low training error is not the end goal—what matters is performance on **new data**.
A simple way to estimate generalization is a **train/test split**.

- **Training error:** computed on the data used to fit the model.
- **Test error:** computed on held-out data; a proxy for generalization error.

We will use metrics such as **MSE/RMSE** (in the same units as $Y$) and sometimes $R^2$.
...
> **Note (ML vs inference).** Coefficients can be interpreted causally only under strong study-design assumptions.
> For prediction-focused ML, coefficients are often treated as *parameters that improve generalization*; when predictors are correlated, individual coefficients can change a lot without changing predictive accuracy much.
...
# sklearn uses "neg_mean_squared_error" so we flip the sign
cv_mse = -cross_val_score(model, X, y, cv=cv, scoring="neg_mean_squared_error")
{"CV_MSE_mean": float(np.mean(cv_mse)), "CV_MSE_sd": float(np.std(cv_mse))}
```
