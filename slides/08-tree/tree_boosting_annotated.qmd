...
![Bagging vs boosting: training is **parallel** vs **sequential** (ESL Ch. 10 schematic).](boosting_figs/boosting_p29.png){fig-align="center" width="85%"}

![Prediction: bagging averages; boosting takes a *weighted* average / vote.](boosting_figs/boosting_p30.png){fig-align="center" width="85%"}
...
::: {.callout-note appearance="minimal"}
### AdaBoost (binary classification) — concept-level algorithm (ESL Ch. 10)
...
![AdaBoost intuition: train weak learners on *reweighted* data, then take a weighted vote.](boosting_figs/boosting_p07.png){fig-align="center" width="85%"}

::: {.callout-tip appearance="minimal"}
### Instructor notes: why AdaBoost looks the way it does (ESL Ch. 10)

- **What does the weight distribution mean?**  
  Think of $w_i^{(b)}$ as defining a *temporary training distribution*: points with larger weight are the ones the next classifier is asked to fit well.

- **Why do we need a "weak learner" (better than chance)?**  
  If the weighted error $\epsilon_b < 1/2$, then
  $$
  \alpha_b = \tfrac12\log\Big(\tfrac{1-\epsilon_b}{\epsilon_b}\Big) > 0,
  $$
  so the learner gets a **positive vote weight**.  
  If $\epsilon_b > 1/2$, the classifier is worse than random guessing; we can flip its predictions to get error $1-\epsilon_b$ and make $\alpha_b$ positive.

- **Why does the weight update use an exponential?**  
  With $y_i f_b(x_i)\in\{-1,+1\}$:
  $$
  \exp\big(-\alpha_b y_i f_b(x_i)\big)=
  \begin{cases}
  e^{-\alpha_b} & \text{if } y_i=f_b(x_i) \;\; (\text{correct}),\\
  e^{+\alpha_b} & \text{if } y_i\ne f_b(x_i) \;\; (\text{wrong}).
  \end{cases}
  $$
  So correctly classified points get *down-weighted* and misclassified points get *up-weighted* — exactly the “focus on hard cases” intuition.

- **Why is $\alpha_b$ a log-odds of the error? (a 1-line derivation)**  
  At step $b$ the normalization constant is
  $$
  Z_b = \sum_{i=1}^n w_i^{(b)}\exp\big(-\alpha_b y_i f_b(x_i)\big)
      = (1-\epsilon_b)e^{-\alpha_b} + \epsilon_b e^{\alpha_b}.
  $$
  Minimizing $Z_b$ over $\alpha_b$ gives
  $$
  \alpha_b = \tfrac12\log\Big(\tfrac{1-\epsilon_b}{\epsilon_b}\Big).
  $$

- **Why does training error often drop quickly? (upper-bound story)**  
  The exponential loss upper-bounds the 0/1 error, since
  $$
  \mathbf{1}\{y_i\ne \mathrm{sign}(F_B(x_i))\}\le \exp\big(-y_i F_B(x_i)\big).
  $$
  AdaBoost decreases the average exponential loss, and hence decreases an **upper bound** on training misclassification.

- **Practical punchline**  
  AdaBoost is not directly minimizing misclassification error; it is minimizing an upper bound (exponential loss). That’s why we typically choose $B$ by **validation / CV** (early stopping) and often use a small learning rate.
:::

![Training misclassification can hit 0 while the exponential loss keeps decreasing (ESL Fig. 10.3).](boosting_figs/boosting_p19.png){fig-align="center" width="85%"}
...
::: {.callout-note appearance="minimal"}
### Instructor notes: gradient boosting = forward stage-wise additive modeling (ESL Ch. 10)

A useful “big picture” view is that boosting builds an **additive model**
$$
F_B(x)=\sum_{b=1}^B \beta_b\, b(x;\gamma_b),
$$
where each $b(x;\gamma_b)$ is a simple base learner (often a small tree).

Instead of optimizing all $B$ terms at once (hard), we do it **stage-by-stage**:
$$
(\beta_b,\gamma_b)=\arg\min_{\beta,\gamma}\sum_{i=1}^n
L\big(y_i,\;F_{b-1}(x_i)+\beta\, b(x_i;\gamma)\big).
$$

The “pseudo-residuals” $r_{ib}$ are just the **negative gradient** of the loss, i.e.,
the direction that most decreases the loss at the current fit.
:::
...
::: {.callout-note appearance="minimal"}
### AdaBoost and logistic regression scores (ESL Ch. 10)

AdaBoost itself is often written with the **exponential loss** $\exp(-yF(x))$ and $y\in\{-1,+1\}$.
At the population level, the minimizer satisfies
$$
F(x)=\tfrac12\log\frac{\Pr(Y=1\mid x)}{\Pr(Y=-1\mid x)}.
$$
So the AdaBoost score $F(x)$ is (half of) a **log-odds**, and you can map it to a probability via
$$
\Pr(Y=1\mid x)=\frac{e^{2F(x)}}{1+e^{2F(x)}}.
$$

This connection is one reason ESL describes boosting as fitting an *additive logistic regression* model.
:::
...
::: {.callout-note appearance="minimal"}
### Mapping ideas to common software parameters

Different libraries use different names, but the concepts match:

- number of boosting iterations $B$  ↔ `n.trees` (R **gbm**), `n_estimators` (sklearn), `num_boost_round` (XGBoost)
- learning rate $\nu$  ↔ `shrinkage` / `learning_rate`
- tree complexity  ↔ `interaction.depth` (gbm), `max_depth` / `num_leaves`
- row subsampling (stochastic boosting)  ↔ `bag.fraction` / `subsample`
- feature subsampling  ↔ `colsample_bytree`, `max_features`
:::
