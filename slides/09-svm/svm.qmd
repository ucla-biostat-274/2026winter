...
- Non-separable data. Sometimes the data are not separable by a linear boundary. This is often the case, unless $N < p$.
...
$$
computes the inner products needed for $d$-degree polynomials. **Note** In `kernlab`: <https://rdrr.io/cran/kernlab/man/dots.html>, the `scale_factor` is the `scale` parameter in Polynomial kernel $k(x,x') = (\text{scale} <x, x'> + \text{offset})^{\text{degree}}$: The scaling parameter of the polynomial and tangent kernel is a convenient way of normalizing patterns without the need to modify the data itself.  

    For example, for degree $d=2$ and $p=2$ features, 
\begin{eqnarray*}
K(x, x') &=& (1 + \langle x, x' \rangle)^2 \\
&=& 1 + 2x_1x_1' + 2 x_2 x_2' + (x_1x_1)^2 + (x_2x_2')^2 + 2x_1x_1'x_2x_2' \\
&=& \left\langle \begin{pmatrix} 1 \\ \sqrt{2} x_1 \\ \sqrt{2} x_2 \\ x_1^2 \\ x_2^2 \\ \sqrt{2} x_1 x_2 \end{pmatrix}, \begin{pmatrix} 1 \\ \sqrt{2} x_1' \\ \sqrt{2} x_2' \\ x_1^{'2} \\ x_2^{'2} \\ \sqrt{2} x_1' x_2' \end{pmatrix} \right\rangle
\end{eqnarray*}
