...
## Classification in this course: a modern ML baseline

Many of you have already learned classification (especially logistic regression) as a *statistical* tool: coefficient interpretation, standard errors, hypothesis tests, and confounding.
In this course, we start from a **machine learning** perspective.

- **Task:** supervised learning (binary and multiclass classification).
- **Goal:** accurate prediction on *new, unseen* data (**generalization**).
- **Outputs that matter:** class labels *and* well-calibrated probabilities.

### The supervised learning pipeline

We will repeatedly use the same template throughout the course:

1. **Data:** features $X \in \mathbb{R}^{n\times p}$ and labels $y$.
2. **Model family:** a scoring function $s_\theta(x)$ and a link to probabilities (e.g., sigmoid/softmax).
3. **Loss:** a *proper* loss for probabilities (e.g., **log loss / cross-entropy**).
4. **Optimization / learning:** choose $\hat\theta$ to minimize training loss (often via gradient-based methods).
5. **Evaluation:** estimate test performance with a train/test split or cross-validation.

> In machine learning, the “assumptions” behind a model are best viewed as *inductive bias*—a preference for simpler decision boundaries that often generalize well.
...
Default = pd.read_csv("./slides/data/Default.csv")
Default
```
...
Regularized (modern) logistic regression adds a penalty to control complexity, e.g. **$\ell_2$ (ridge) logistic regression**:
$$
\min_\beta\ \mathcal{L}(\beta) + \lambda\|\beta\|_2^2.
$$
Most ML libraries (including scikit-learn) fit *regularized* logistic regression by default.
...
# Preprocess: one-hot encode categorical vars and scale numeric vars
...
pipe_logit = Pipeline(steps = [
  ...
  # scikit-learn uses regularized logistic regression by default (L2 penalty)
...
## Prediction and evaluation (train vs test)
...
# Rebuild fresh models to avoid accidental leakage from earlier full-data fits
models = {
    "Null": DummyClassifier(strategy="most_frequent"),
    "Logit": Pipeline([( "preprocess", preprocess), ("model", LogisticRegression(max_iter=1000))]),
    "LDA":   Pipeline([( "preprocess", preprocess), ("model", LinearDiscriminantAnalysis())]),
    "QDA":   Pipeline([( "preprocess", preprocess), ("model", QuadraticDiscriminantAnalysis())]),
    "NB":    Pipeline([( "preprocess", preprocess), ("model", GaussianNB())]),
    "KNN":   Pipeline([( "preprocess", preprocess), ("model", KNeighborsClassifier(n_neighbors=5))]),
}
...
for name, m in models.items():
    ...
    idx_yes = np.where(m.classes_ == "Yes")[0][0]
    p_yes = proba[:, idx_yes]
    ...
    rows.append({
        ...
        "LogLoss": log_loss(y_test, proba, labels=["No", "Yes"]),
        "AUC": roc_auc_score(y_test_bin, p_yes),
        # PR-AUC is often more informative under heavy class imbalance
        "PR_AUC": average_precision_score(y_test_bin, p_yes),
    })
...
for(nm in names(pred_probs)) {
  roc_object <- roc(Default_test$default, pred_probs[[nm]])
  auc_c <- auc(roc_object)[1]
  print(
    ggroc(roc_object) +
      ggtitle(str_c(nm, " ROC on test with AUC = ", round(auc_c, 4))) +
...
for name, m in models.items():
    plt.figure()
    RocCurveDisplay.from_estimator(m, X_test, y_test, name=name)
    plt.show()
```

:::

### Model selection: cross-validation (example)

Logistic regression has a key hyperparameter: the regularization strength (`C` in scikit-learn).
In ML, we usually choose such hyperparameters by cross-validation.
...
cv_auc = cross_val_score(logit_model, X, y_bin, cv=cv, scoring="roc_auc")
{"CV_AUC_mean": float(np.mean(cv_auc)), "CV_AUC_sd": float(np.std(cv_auc))}
```

:::

See [sklearn.metrics](https://scikit-learn.org/stable/modules/model_evaluation.html) for other commonly used metrics for classification tasks.
